                                           ...
                                           bool add_gelu = false)
{
    ...
    for (int j = 0; j < input_size; j++) {
        __half2 sum[2];
#pragma unroll
        for (int t = 0; t < 2; t++) { sum[t] = __float2half2_rn(0.f); }

        {
            ...
            while (wid < hidden_dim) {
                ...
                __half2 vals_f[INPUT_TILE1 * loop_unroll];
                {
                    ...
                    val_h[0] = vals_cast[(j) * (hidden_half << block_reduce) + (wid >> 1)];
                    val_h[1] = vals_cast[(j) * (hidden_half << block_reduce) + (wid >> 1) + 1];
...
#pragma unroll
                        ...
                        unsigned q_index = (wid - merge_hidden_total) + (col << 2) * total_hidden +
                                           (unsigned)(blockIdx.x / outputBlocks) * merge_hidden;

                        float scale_data[4];
#pragma unroll
                        for (int k = 0; k < loop_unroll; k++)
                            scale_data[k] = shared_quantize_scale[(((q_index + k * total_hidden) /
                                                                    quantization_stride)
                                                                   << merge_count) +
                                                                  w_index];
                        ...
                        for (int k = 0; k < loop_unroll; k++) {
                            ...
                            __half* weight_data = reinterpret_cast<__half*>(&weight_h[k << 1]);
...
#pragma unroll
        ...
        }
        {
...

            {
                ...
                sum[1] = (partial_result[(lane << 1) * (WARP_SIZE + 2) + (gid << 1) + 1]);
